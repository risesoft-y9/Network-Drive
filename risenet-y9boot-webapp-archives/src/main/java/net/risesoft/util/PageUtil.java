...
public class PageUtil {
    ...
    public <T> SearchPage<T> page(String sql, Map<String, Object> sqlMap, RowMapper<T> rowMapper, String countSql,
        ...
        if (totalSize == 0) {
            return SearchPage.<T>builder().rows(new ArrayList<>()).currpage(0).size(0).totalpages(0).total(0).build();
        }
        ...
        return SearchPage.<T>builder().rows(content).total(totalSize).totalpages(totalPage).currpage(currPage)
            .size(size).build();
    }
     ...
     */
    @SuppressWarnings("deprecation")
    public <T> SearchPage<T> page(String sql, Object[] queryArgs, RowMapper<T> rowMapper, String countSql,
