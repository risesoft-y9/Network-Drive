...
public class ArchivesServiceImpl implements ArchivesService {
    ...
    @Override
    public SearchPage<Archives> listArchives(String categoryId, Integer fileStatus, Boolean isDeleted, int page,
        int rows) {
        Pageable pageable = PageRequest.of(page - 1, rows, Sort.by(Sort.Direction.DESC, "createTime"));
        Page<Archives> pageList =
            archivesRepository.findByCategoryIdAndFileStatusAndIsDeleted(categoryId, fileStatus, isDeleted, pageable);
    ...
    public Y9Result<String> createArchivesNo(String categoryId, Long[] ids) {
        ...
        for (Long id : ids) {
            Archives archives = this.findByArchives_id(id);
            List<ArchivesNumberRules> rulesList =
                archivesNumberRulesRepository.findByCategoryMark(dataCatalog.getCustomId());
            if (null != rulesList && rulesList.size() > 0) {
                String archiveNo = "";
                for (ArchivesNumberRules rules : rulesList) {
                    // TODO 根据规则生成档案号
    ...
    @Override
    public List<Archives> findByArchivesIdIn(Long[] ids) {
        return archivesRepository.findByArchivesIdIn(ids);
    }
    ...
    public Y9Result<Map<String, Object>> checkArchives(String processName, Long[] archivesId) {
        ...
        for (Long id : archivesId) {
            ...
            if (null != archives) {
                ...
                // 【GD-1-1】档案文件上传人是否存在
                ArchivesTestingInfo testingInfo = new ArchivesTestingInfo();
