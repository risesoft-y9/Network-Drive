...
public class DataAssetsServiceImpl implements DataAssetsService {
    ...
    @Override
    public Page<DataAssets> searchPage(String categoryId, String name, String code, Integer status, int page,
        int size) {
        if (page < 0) {
            page = 1;
        }
        ...
        DataAssetsSpecification spec =
            new DataAssetsSpecification(categoryId, name, Y9LoginUserHolder.getTenantId(), code, false, status);
    ...
    private String getIdCode(Long id, String name) {
        ...
        IdcodeRegResult result = Four.m407(Config.MAIN_CODE, "60", 10167, "11000000", _id, "", name,
            CodePayTypeEnum.REGISTER.getValue(), Config.GOTO_URL, Config.SAMPLE_URL);
