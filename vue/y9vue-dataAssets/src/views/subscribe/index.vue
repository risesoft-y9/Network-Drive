        ...
        >
            <template v-slot:rightContainer>
                ...
                <div v-if="currData.id">
                    <!-- 列表 -->
                    <DataView :currTreeNodeInfo="currData" />
                </div>
            </template>
        </fixedTreeModule>
    </div>
...
<script lang="ts" setup>
    ...
    import DataView from './comps/DataView.vue';
    ...
    const treeApiObj = ref({
        topLevel: async () => {
            return dataCatalogTree2({parentId: null});
        }, //顶级（一级）tree接口,
        childLevel: {
            //子级（二级及二级以上）tree接口
            api: dataCatalogTree2,
            params: {}
        },
        search: {
            //搜索接口及参数
            api: searchCatelogTree2,
            params: {}
        }
    });
</script>
