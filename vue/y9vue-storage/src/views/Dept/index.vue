  ...
  <y9Card :showHeader="false">
    <div class="toolbar">
      <div class="toolbar-left">
        ...
        <!-- <el-button :size="fontSizeObj.buttonSize"
:style="{ fontSize: fontSizeObj.baseFontSize }" v-if="multipleSelection.length === 1" :disabled="notCurrentSelectedOwner" class="global-btn-second" v-on:click="share" plain>
    ...
    <el-form class="formClass" ref="fileForm" :model="formData" :rules="rules">
      ...
      <y9Table ref="deptTable" :config="y9TableConfig" v-loading="loading" :element-loading-text="loadingTitle"
          ...
          <el-row v-else @mouseenter="titleHover(row.id)" @mouseleave="titleLeave(row.id)">
            ...
            <el-col :span="5">
              <div class="optButtonCss" v-if="optButtonShow == row.id">
                ...
                <!-- <el-tooltip v-if="row.fileType != 0" class="box-item" effect="light" content="共享"
                    placement="top-start">
                    <i class="ri-share-line" @click="share"></i>
                  </el-tooltip> -->
                <el-tooltip v-if="row.fileType == 0 && positionId == row.userId" class="box-item" effect="light"
        ...
        <template #place="{ row, column, index }">
          ...
          <div v-else>
            <router-link v-if="row.parentFileNode"
              v-bind:to="{ path: '/dept/fileList/all', query: { parentId: row.parentId, listType: listType } }">
              {{ row.parentFileNode.name }}
            </router-link>
...
//调整表格高度适应屏幕
const tableHeight = ref(useSettingStore().getWindowHeight - 260 - 28);

window.onresize = () => {
  return (() => {
    tableHeight.value = useSettingStore().getWindowHeight - 260 - 28;
  })();
};
const data = reactive({
  positionId: '',
...
let {
  positionId, y9TableConfig, loadingTitle, fileObject, audioArray, poster, fileId, fileName, fileUrl, uploadLoading, percentage, fileForm, isEmptyData, nameSign, editIndex, formData, deptTable, notCurrentSelectedOwner,
...
function loadList() {
  ...
  FileApi.list(props.parentId, searchKey.value, props.fileNodeType, 'dept', orderProp.value, orderAsc.value).then(res => {
    loading.value = false;
