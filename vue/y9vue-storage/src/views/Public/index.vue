  ...
  <y9Card :showHeader="false">
    <div class="toolbar">
      <div class="toolbar-left">
        <el-upload v-if="roleType === 'manage'" action="" class="upload-div" :show-file-list="false" multiple
          v-bind:http-request="uploadFile">
          <el-button :size="fontSizeObj.buttonSize"
                     :style="{ fontSize: fontSizeObj.baseFontSize }" class="global-btn-main" type="primary"><i class="ri-upload-cloud-2-line"></i>{{ $t('上传') }}</el-button>
        </el-upload>

        <el-button :size="fontSizeObj.buttonSize"
                   :style="{ fontSize: fontSizeObj.baseFontSize }" v-if="!fileNodeType && roleType === 'manage'" class="global-btn-second" v-on:click="createFolder"
          plain>
        ...
        <el-button :size="fontSizeObj.buttonSize"
                   :style="{ fontSize: fontSizeObj.baseFontSize }" v-if="multipleSelection.length && roleType === 'manage'" :disabled="notCurrentSelectedOwner"
          class="global-btn-second" v-on:click="deleteSelect" plain>
          <i class="ri-delete-bin-line"></i> {{ $t('删除') }}
        </el-button>
        <el-button  :size="fontSizeObj.buttonSize"
                    :style="{ fontSize: fontSizeObj.baseFontSize }" v-if="multipleSelection.length && roleType === 'manage'" :disabled="notCurrentSelectedOwner"
          class="global-btn-second" v-on:click="move" plain>
          <i class="ri-login-box-line"></i> {{ $t('移动到') }}
        </el-button>
        <el-button
            :size="fontSizeObj.buttonSize"
            :style="{ fontSize: fontSizeObj.baseFontSize }"
          v-if="(multipleSelection.length === 1 && roleType === 'manage' && multipleSelection[0].fileType != '0')"
          :disabled="notCurrentSelectedOwner" class="global-btn-second" v-on:click="publicTo" plain>
          <i class="ri-share-fill"></i>{{ $t('公开至') }}
        </el-button>
        <el-button :size="fontSizeObj.buttonSize"
                   :style="{ fontSize: fontSizeObj.baseFontSize }" v-if="multipleSelection.length === 1 && roleType === 'manage'" :disabled="notCurrentSelectedOwner"
          v-on:click="renameOutBtn" class="global-btn-second" plain>
          <i class="ri-edit-2-line"></i>{{ $t('重命名') }}
        </el-button>
        <el-button :size="fontSizeObj.buttonSize"
                   :style="{ fontSize: fontSizeObj.baseFontSize }" class="global-btn-second" @click="refresh" plain><i class="ri-refresh-line"></i>{{ $t('刷新') }}</el-button>
      </div>
      <div class="toolbar-right">
        <el-form :inline="true">
          ...
          <el-form-item :label="$t('创建时间')">
            <el-date-picker :teleported="false" v-model="selectedDate" type="daterange" :range-separator="$t('至')" :shortcuts="shortcuts"
              :start-placeholder="$t('开始时间')" :end-placeholder="$t('结束时间')" format="YYYY-MM-DD" value-format="YYYY-MM-DD"
              @change="selectdDate()"></el-date-picker>
          </el-form-item>
          <el-form-item>
            <el-button :size="fontSizeObj.buttonSize"
                       :style="{ fontSize: fontSizeObj.baseFontSize }" class="global-btn-second" @click="toSearchView"><i class="ri-search-line"></i>{{ $t('查询') }}</el-button>
          </el-form-item>
          <el-form-item>
            <el-dropdown>
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item>
                    <el-radio v-model="orderProp" label="FILE_NAME"
                      @click.native="changeOrder($event, 'FILE_NAME')">{{ $t('文件名') }}</el-radio>
                  </el-dropdown-item>
                  <el-dropdown-item>
                    <el-radio v-model="orderProp" label="FILE_SIZE"
                      @click.native="changeOrder($event, 'FILE_SIZE')">{{ $t('文件大小') }}</el-radio>
                  </el-dropdown-item>
                  <el-dropdown-item>
                    <el-radio v-model="orderProp" label="CREATE_TIME"
                      @click.native="changeOrder($event, 'CREATE_TIME')">{{ $t('创建时间') }}</el-radio>
                  </el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
      ...
      <y9Table ref="multipleTable" :config="y9TableConfig" v-loading="loading" :element-loading-text="loadingTitle"
        ...
        <template #name="{ row, column, index }">
          <el-form-item prop="name" v-if="editIndex === index">
            ...
            <el-button :size="fontSizeObj.buttonSize"
                       :style="{ fontSize: fontSizeObj.baseFontSize }" style="margin-left: 14px;" type="primary" class="global-btn-main" @click="saveData(fileForm)"><i
                class="ri-check-line"></i></el-button>
            <el-button :size="fontSizeObj.buttonSize"
                       :style="{ fontSize: fontSizeObj.baseFontSize }" class="global-btn-second" @click="cancalData(fileForm)" plain><i class="ri-close-line"></i></el-button>
          </el-form-item>
          <el-row v-else @mouseenter="titleHover(row.id)" @mouseleave="titleLeave(row.id)">
            ...
            <el-col :span="5">
              <div class="optButtonCss" v-if="(roleType == 'manage' && optButtonShow == row.id)">
                <template v-if="row.filePassword == '' || row.filePassword == null">
                  <el-tooltip v-if="(row.fileType != 0)" class="box-item" effect="light" :content="$t('公开记录')"
                    placement="top-start">
                    <i class="ri-team-line" @click="openPublicRecord(row.id)"></i>
                  </el-tooltip>
                  <el-tooltip v-if="(row.fileType != 0)" class="box-item" effect="light" :content="$t('下载记录')"
                    placement="top-start">
                  ...
                  <el-tooltip class="box-item" effect="light" :content="$t('移动至')" placement="top-start">
                    <i class="ri-login-box-line" @click="move"></i>
                  </el-tooltip>
                  <el-tooltip v-if="row.fileType != 0" @click="publicTo" class="box-item" effect="light" :content="$t('公开至')"
                    placement="top-start">
                    <i class="ri-share-line" @click="publicTo"></i>
                  </el-tooltip>
                </template>
                <el-tooltip v-if="row.fileType == 0 && y9UserInfo.personId == row.userId" class="box-item" effect="light"
                  :content="$t('加密')" placement="top-start">
        ...
        <template #place="{ row, column, index }">
          <div v-if="roleType == 'manage'">
            ...
            <div v-else>
              <router-link v-if="row.parentFileNode"
                v-bind:to="{ path: '/public/manage/index', query: { parentId: row.parentId, roleType: roleType } }">
                {{ row.parentFileNode.name }}
              </router-link>
          ...
          <div v-else>
            ...
            <div v-else>
              <router-link v-if="row.parentFileNode"
                v-bind:to="{ path: '/public/fileList/all', query: { parentId: row.parentId, roleType: roleType } }">
                {{ row.parentFileNode.name }}
              </router-link>
...
//调整表格高度适应屏幕
const tableHeight = ref(useSettingStore().getWindowHeight - 260 - 45);

window.onresize = () => {
  return (() => {
    tableHeight.value = useSettingStore().getWindowHeight - 260 - 45;
  })();
};
const data = reactive({
  y9UserInfo: {},
  ...
  y9TableConfig: {
    ...
    columns: [
      ...
      { title: computed(() => t("创建日期")), key: "createTime", width: '170', },
    ],
    tableData: []
  },
  dialogConfig: {
...
let {
  y9UserInfo, y9TableConfig, loadingTitle, fileObject, audioArray, poster, fileId, fileName, fileUrl, uploadLoading, percentage, fileForm, isEmptyData, nameSign, editIndex, formData, multipleTable, notCurrentSelectedOwner, buttonMore,
  recursiveToRootFileNodeList, orderProp, orderAsc, searchKey, multipleSelection, loading,
  optSign, sharePersons, startTime, endTime, recordType, optButtonShow,
  moveFileIds, editFileNode, dialogConfig, showHeader, treeSelectedData, treeApiObj, listType, selectedDate, backSign
} = toRefs(data);
...
watch(() => [props.parentId, props.fileNodeType, props.roleType],
...
function loadList() {
  ...
  if (props.roleType == 'manage') {
    FileApi.manageList(props.parentId, searchKey.value, props.fileNodeType, startTime.value, endTime.value, 'public', orderProp.value, orderAsc.value).then(res => {
      loading.value = false;
    ...
    FileApi.publicList(props.parentId, searchKey.value, props.fileNodeType, startTime.value, endTime.value, 'public', orderProp.value, orderAsc.value).then(res => {
      loading.value = false;
      y9TableConfig.value.tableData = res.data.subFileNodeList;
      recursiveToRootFileNodeList.value = res.data.recursiveToRootFileNodeList;
    });
...
async function backSuperior() {
  ...
  if (res.data != null) {
    ...
    if (props.roleType == 'manage') {
      router.push({ path: '/manage/index', query: { parentId: res.data.parentId, roleType: props.roleType } });
    } else {
      router.push({ path: '/public/fileList/all', query: { parentId: res.data.parentId, roleType: props.roleType } });
    }
  }
}
