  ...
  <y9Card :showHeader="false">
    <div class="toolbar">
      <div class="toolbar-left">
        <el-upload v-if="parentId !== 'shared'" action="" class="upload-div" :show-file-list="false" multiple
        ...
        <el-button :size="fontSizeObj.buttonSize"
                   :style="{ fontSize: fontSizeObj.baseFontSize }" v-if="!fileNodeType && parentId !== 'shared'" class="global-btn-second" v-on:click="createFolder"
          plain>
        ...
        <el-button :size="fontSizeObj.buttonSize"
                   :style="{ fontSize: fontSizeObj.baseFontSize }" v-if="multipleSelection.length === 1" :disabled="notCurrentSelectedOwner" class="global-btn-second"
          v-on:click="share" plain>
            ...
            <el-dropdown>
              <span class="el-dropdown-link">
                <el-button class="global-btn-second" :size="fontSizeObj.buttonSize"
                     :style="{ fontSize: fontSizeObj.baseFontSize }"><i class="ri-arrow-up-down-line"></i>{{ $t('排序') }}</el-button>
              </span>
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item>
                  ...
                  <el-dropdown-item>
                    <el-radio v-if="listType !== 'shared'" v-model="orderProp" label="UPDATE_TIME"
                      @click.native="changeOrder($event, 'UPDATE_TIME')">
                      {{ $t('修改时间') }}
                    </el-radio>
                    <el-radio v-else v-model="orderProp" label="CREATE_TIME"
    ...
    <div class="nav">
      <div class="location">
        <span v-if="capacityShow">{{ $t('存储空间') }}：{{ remainingLength }}/{{ capacitySize }}</span>
      ...
      <y9Table ref="multipleTable" :config="y9TableConfig" v-loading="loading" :element-loading-text="loadingTitle"
        ...
        <template #name="{ row, column, index }">
          <el-form-item prop="name" v-if="editIndex === index">
            ...
            <el-button style="margin-left: 14px;" type="primary" @click="saveData(fileForm)" class="global-btn-main" :size="fontSizeObj.buttonSize"
                     :style="{ fontSize: fontSizeObj.baseFontSize }"><i
                class="ri-check-line"></i></el-button>
            <el-button @click="cancalData(fileForm)" class="global-btn-second" plain :size="fontSizeObj.buttonSize"
                     :style="{ fontSize: fontSizeObj.baseFontSize }"><i
            ...
            <el-col :span="6">
              <div class="optButtonCss" v-if="optButtonShow == row.id">
                <template v-if="row.filePassword == '' || row.filePassword == null">
                  ...
                  <el-tooltip v-if="row.fileType != 0" class="box-item" effect="light" :content="$t('生成直链')"
                    placement="top-start">
                    <i class="ri-links-fill" @click="createLink(row)"></i>
                  </el-tooltip>
                </template>
                <el-tooltip class="box-item" effect="light" :content="$t('共享')" placement="top-start">
                    <i class="ri-share-line" @click="fileShare(row)"></i>
                  </el-tooltip>
                <el-tooltip v-if="row.fileType == 0 && parentId !== 'shared'" class="box-item" effect="light" :content="$t('加密')" placement="top-start">
...
const data = reactive({
  ...
  y9TableConfig: {
    ...
    columns: [
      ...
      { title: computed(() => t("文件名")), key: "name", align: "left", width: '900', slot: 'name' },
      ...
      { title: computed(() => t("日期")), key: "time", width: '170', slot: 'time', headerSlot: 'fileDate' },
    ],
    tableData: []
  },
  dialogConfig: {
        ...
        if (dialogConfig.value.type == 'share') {
          if (sharePersons.value.length > 0) {
            ...
            FileNodeShareApi.share(shareFileNodeIdArr, orgUnitIdArr).then(() => {
              ElMessage({ type: "success", message: t("共享成功，文件会移动至'共享空间'中"), offset: 65 });
              loadList();
              resolve()
            });
          } else {
...
let {
  y9TableConfig, loadingTitle, fileObject, audioArray, poster, fileId, fileName, fileUrl, uploadLoading, percentage, fileForm, isEmptyData, nameSign, editIndex, formData, multipleTable, notCurrentSelectedOwner,
...
watch(() => [props.parentId, props.fileNodeType],
  ([pId, fileNodeType]) => {
    if (fileNodeType) {
      loadList();
    }
