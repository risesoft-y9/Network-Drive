        ...
        <template #place="{ row, column, index }">
          ...
          <div v-else>
            <router-link v-if="row.parentFileNode"
              v-bind:to="{ path: '/reportManage/index', query: { parentId: row.parentId, roleType: roleType } }">
              {{ row.parentFileNode.name }}
            </router-link>
...
async function backSuperior() {
  ...
  if (res.data != null) {
    if (res.data.parentId == 'report') {
      ...
      router.push({ path: '/reportManage/index', query: { parentId: res.data.parentId, roleType: props.roleType } });
  }
}
