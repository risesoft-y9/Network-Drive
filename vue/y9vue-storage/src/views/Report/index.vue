    ...
    <div class="toolbar">
      <div class="toolbar-left">
        <el-button :size="fontSizeObj.buttonSize"
                   :style="{ fontSize: fontSizeObj.baseFontSize }" style="margin-right: 12px;" class="global-btn-main" type="primary" @click="uploadBtn"><i
            class="ri-upload-cloud-2-line"></i>{{ $t('上传') }}</el-button>
        <el-upload v-show="false" ref="upload" action="" class="upload-div" :show-file-list="false" multiple
          v-bind:http-request="uploadFile">
          <el-button :size="fontSizeObj.buttonSize"
                     :style="{ fontSize: fontSizeObj.baseFontSize }" class="global-btn-main" ref="uploadShowBtn" type="primary"><i
              class="ri-upload-cloud-2-line"></i>{{ $t('上传') }}</el-button>
        </el-upload>
        ...
        <el-button :size="fontSizeObj.buttonSize"
                   :style="{ fontSize: fontSizeObj.baseFontSize }" v-if="multipleSelection.length" :disabled="notCurrentSelectedOwner" class="global-btn-second"
          v-on:click="deleteSelect" plain>
          <i class="ri-delete-bin-line"></i> {{ $t('删除') }}
        </el-button>
        <el-button :size="fontSizeObj.buttonSize"
                   :style="{ fontSize: fontSizeObj.baseFontSize }" v-if="multipleSelection.length" :disabled="notCurrentSelectedOwner" class="global-btn-second"
          v-on:click="move" plain>
          <i class="ri-login-box-line"></i> {{ $t('移动到') }}
        </el-button>
        <el-button :size="fontSizeObj.buttonSize"
                   :style="{ fontSize: fontSizeObj.baseFontSize }" v-if="multipleSelection.length === 1" :disabled="notCurrentSelectedOwner" v-on:click="renameOutBtn"
          class="global-btn-second" plain>
      ...
      <y9Table ref="multipleTable" :config="y9TableConfig" v-loading="loading" :element-loading-text="loadingTitle"
        ...
        <template #name="{ row, column, index }">
          <el-form-item prop="name" v-if="editIndex === index">
            ...
            <el-button :size="fontSizeObj.buttonSize"
                       :style="{ fontSize: fontSizeObj.baseFontSize }" class="global-btn-second" @click="cancalData(fileForm)" plain><i
            ...
            <el-col :span="5">
              <div class="optButtonCss" v-if="optButtonShow == row.id">
                <template v-if="row.filePassword == '' || row.filePassword == null">
                  <el-tooltip v-if="row.fileType != 0" class="box-item" effect="light" :content="$t('重命名')"
                    placement="top-start">
                    <i class="ri-edit-2-line" @click="renameListBtn(row)"></i>
                  </el-tooltip>
                  <el-tooltip v-if="row.fileType != 0" class="box-item" effect="light" :content="$t('删除')" placement="top-start">
                    <i class="ri-delete-bin-line" @click="deleteOne(row.id)"></i>
                  </el-tooltip>
                  <el-tooltip v-if="row.fileType != 0" class="box-item" effect="light" :content="$t('下载')" placement="top-start">
                    <i class="ri-download-2-line" @click="downloadFile(row.id)"></i>
                  </el-tooltip>
                  <el-tooltip v-if="row.fileType != 0" class="box-item" effect="light" :content="$t('移动至')"
                    placement="top-start">
        ...
        <template #place="{ row, column, index }">
          ...
          <div v-else>
            <router-link v-if="row.parentFileNode"
              v-bind:to="{ path: '/report/index', query: { parentId: row.parentId, roleType: roleType } }">
              {{ row.parentFileNode.name }}
            </router-link>
            <router-link v-else to="/report/index">{{ $t('全部文件') }}</router-link>
          </div>
        </template>
      </y9Table>
    </el-form>
    ...
    columns: [
      {
        type: "selection", width: '55', selectable: (row) => {
          if (!row.filePassword&&row.fileType!=0) {
            return true
          }
        }
      },
  ...
  dialogConfig: {
    ...
    onOk: (newConfig) => {
      return new Promise(async (resolve, reject) => {
        if (dialogConfig.value.type == 'moveNode') {
          if (treeSelectedData.value) {
            ...
            if (treeSelectedData.value.id == 'report') {
              ElMessage({ type: "error", message: t("不能将文件最外层的主目录，必须选择一个文件夹移动！"), offset: 65 });
              reject();
              return;
            }
            if (pathValid) {
...
let {
  uploadShowBtn, y9UserInfo, y9TableConfig, loadingTitle, fileObject, audioArray, poster, fileId, fileName, fileUrl, uploadLoading, percentage, fileForm, isEmptyData, nameSign, editIndex, formData, multipleTable, notCurrentSelectedOwner, buttonMore,
  recursiveToRootFileNodeList, orderProp, orderAsc, searchKey, multipleSelection, loading,
  optSign, optButtonShow,
...
function loadList() {
  ...
  FileApi.list(props.parentId, searchKey.value, '', props.listType, orderProp.value, orderAsc.value).then(res => {
    loading.value = false;
...
function subList(row) {
  if (row.filePassword != '' && row.filePassword != null) {
    ...
    if (props.roleType == 'manage') {
      ...
      router.push({ path: '/report/index', query: { parentId: row.id, roleType: props.roleType } });
    }
  }
...
async function backSuperior() {
  ...
  if (res.data != null) {
    ...
    if (props.roleType == 'manage') {
      ...
      router.push({ path: '/report/index', query: { parentId: res.data.parentId, roleType: props.roleType } });
    }
  }
}
