            ...
            <el-form ref="viewTypeForm" :model="formData" :rules="rules">
                ...
                >
                    ...
                    <template #opt="{ row, column, index }">
                        ...
                        <div v-else>
                            <template v-if="row.categorySource == 'systemAdd'">
...
<script lang="ts" setup>
    ...
    import { remove, saveOrUpdate, getCategoryList } from '@/api/archives/category';
    ...
    //调整表格高度适应屏幕
    const tableHeight = ref(useSettingStore().getWindowHeight - 323);

    window.onresize = () => {
        return (() => {
            tableHeight.value = useSettingStore().getWindowHeight - 323;
        })();
    };
    const data = reactive({
        editIndex: '',
    ...
    async function getList() {
        ...
        let res = await getCategoryList(page, rows);
    ...
    const delData = (rows) => {
        ElMessageBox.confirm('您确定要删除此门类吗?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
        })
            .then(() => {
                remove(rows.id).then((res) => {
                    if (res.success) {
                        ElMessage({ type: 'success', message: res.msg, offset: 65 });
                        getList();
                    } else {
