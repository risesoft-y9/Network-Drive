            ...
            <y9Card>
                <template #header>
                    <div style="display: inline-flex">
                        <div><i class="ri-arrow-down-s-fill"></i></div>
                        <div style="padding-left: 5px"><i class="ri-archive-line"></i></div>
                        <div style="padding-left: 5px"><span>档案目录</span></div>
                    </div>
                    <!-- <y9Filter :itemList="itemList" itemMarginBottom="0px">
                        <template #refresh>
                            <el-button class="global-btn-second" @click="onRefreshTree">
                                <i class="ri-refresh-line i_medium"></i>
                                <span>刷新</span>
                            </el-button>
                        </template>
                ...
                >
                    ...
                    <!-- <template #actions="{ item }">
                        <slot :item="item" name="actions">
                            <i v-if="item.delete_icon" class="ri-delete-bin-line" @click="onRemoveNode(item)"></i>
                        </slot>
                    </template> -->
                </y9Tree>
            </y9Card>
...
<script lang="ts" setup>
    ...
    const onNodeDblclick = (node) => {
        emits('onTreeDblclick', node);
        console.log('nhhgggh', node);

        let keys = [];
        keys.push(node.id);
        y9TreeRef.value?.setExpandKeys(keys); //设置为高亮节点
    };
...
<style lang="scss" scoped>
    ...
    .fixed {
        ...
        :deep(.y9-card) {
            height: calc(100vh - #{$headerHeight} - #{$headerBreadcrumbHeight} - 45px);
            overflow: hidden;
        }
